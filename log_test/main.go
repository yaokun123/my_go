...
/**
在我们开发程序后，如果有一些问题需要对程序进行调试的时候，日志是必不可少对的，这是我们分析程序问题常用的手段

日志使用：
日志分析，就是根据输入的日志信息，分析挖掘可能的问题，我们使用fmt.Println系列函数也可以达到目的，因为他们也可以把
我们需要的信息输出到终端或者其他文件中。不过fmt.Println系列函数输出的系统比较简单，比如没有时间，也没有源代码的行数等，
对我们排查问题，缺少了很多信息。
